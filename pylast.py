...
class User(_BaseObject):
    ...
    def get_recent_tracks(self, limit = 10):
        ...
        return seq
    def get_tracks_between(self, _from, _to):
        """Returns the played tracks between given dates
        Currently returning max number of 200 tracks
        TODO:paging support
        """
        params= self._get_params()
        params['from']=_from
        ...
        params['limit']=200
        seq=[]
        for track in _collect_nodes(200,self,"user.getRecentTracks",True,params):
            if track.hasAttribute('nowplaying'):
                continue    #to prevent the now playing track from sneaking in here
            title = _extract(track,"name")
            artist = _extract(track,"artist")
